...
class ObjectdetectionModelDetection extends JModelLegacy
{
    ...
    public function __construct($config = array())
    {
        ...
        // ТОЛЬКО загружаем параметры компонента
    ...
    /**
     * Сохраняет загруженное изображение во временную папку пользователя
     */
    private function saveUploadedImage($file)
    {
        $user = JFactory::getUser();
        $userDir = JPATH_ROOT . '/images/com_objectdetection/user_' . $user->id;
        
        // Очищаем предыдущие файлы пользователя ТОЛЬКО при сохранении нового файла
        $this->cleanupPreviousUserFiles();
        ...
        if (move_uploaded_file($file['tmp_name'], $filepath)) {
            return 'images/com_objectdetection/user_' . $user->id . '/' . $filename;
        }
    ...
    /**
     * Сохраняет аннотированное изображение во временную папку пользователя
     */
    ...
    /**
     * Удаляет предыдущие файлы пользователя
     */
    private function cleanupPreviousUserFiles()
    {
        $user = JFactory::getUser();
        $userDir = JPATH_ROOT . '/images/com_objectdetection/user_' . $user->id;
