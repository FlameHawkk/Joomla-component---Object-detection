...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Собираем изображения из DOM (они уже загружены в браузер)
    const images = Array.from(document.querySelectorAll('.result-image')).map(img => img.src);
    ...
    function updateLightbox() {
        const currentImg = document.querySelector('.result-image[src="' + images[currentImageIndex] + '"]');
        ...
        if (currentImg) {
            ...
            fetch(currentImg.src)
                ...
                .then(blob => {
                    cleanupBlobUrl(); // Очищаем предыдущий blob
                    currentBlobUrl = URL.createObjectURL(blob);
                    lightboxDownload.href = currentBlobUrl;
                    
                    const fileName = currentImageIndex === 0 ? 
                        'original_' + (new Date()).getTime() + '.jpg' : 
                        'processed_' + (new Date()).getTime() + '.jpg';
                ...
                .catch(error => {
                    ...
                    lightboxDownload.download = 'image_' + (new Date()).getTime() + '.jpg';
                });
        } else {
            // Fallback
